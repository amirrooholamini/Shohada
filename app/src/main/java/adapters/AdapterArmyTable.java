...
public class AdapterArmyTable extends ArrayAdapter<StructArmyTable> {
    private LayoutInflater inflater;

    public AdapterArmyTable(Context context, ArrayList<StructArmyTable> list) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        final StructArmyTable item = getItem(position);
        final ViewHolder viewHolder;
        if (convertView == null) { // there is empty space
            convertView = inflater.inflate(R.layout.struct_army_table, parent, false);
